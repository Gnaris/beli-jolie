...
export const metadata: Metadata = {
  title: "Marketing — Envoi de mails — Admin",
};
...
// Filtres spécifiques à Marketing : seuls les clients APPROUVÉS sont éligibles
// aux mails marketing (PENDING/REJECTED n'apparaissent pas ici). On expose des
// segments utiles à la cliente pour ses campagnes.
const FILTERS: { value: string; label: string }[] = [
  { value: "ALL",             label: "Tous" },
  { value: "ACCEPTS_MAILS",   label: "Accepte les mails" },
  { value: "REFUSES_MAILS",   label: "Refuse les mails" },
  { value: "HAS_PENDING_CART", label: "Panier abandonné en cours" },
];
...
export default async function MarketingPage({
  ...
  // Ne garder que les segments marketing valides — un param `status` hérité
  // de /admin/clients (PENDING/APPROVED/REJECTED) doit retomber sur « ALL »
  // pour ne pas planter le filtre.
  const VALID_MARKETING_FILTERS = new Set([
    "ALL",
    "ACCEPTS_MAILS",
    "REFUSES_MAILS",
    "HAS_PENDING_CART",
  ]);
  const filterStatus = VALID_MARKETING_FILTERS.has(params.status ?? "")
  ...
  // Page Marketing : uniquement la vue « Mails » (envoi de mails aux clients).
  // La vue « Infos » vit sur /admin/clients. L'annotation large `as ...`
  // conserve les blocs conditionnels `view === "infos"` en aval (chargement
  // du panier notamment) — sinon TypeScript narrow à la valeur littérale et
  // considère `view === "infos"` comme une comparaison impossible.
  const view = "mails" as "infos" | "mails";

  // Base marketing : uniquement les clients APPROUVÉS. Les PENDING/REJECTED
  // ne peuvent recevoir aucun mail marketing (règle validée par les gates mail).
  const marketingBaseWhere: Prisma.UserWhereInput = {
    role: "CLIENT",
    status: "APPROVED",
  };
  ...
  // Chaque filtre projette un WHERE partiel additionnel sur la base marketing.
  function whereForFilter(f: string): Prisma.UserWhereInput {
    switch (f) {
      case "ACCEPTS_MAILS":
        return { ...marketingBaseWhere, acceptsNewsletter: true };
      case "REFUSES_MAILS":
        return { ...marketingBaseWhere, acceptsNewsletter: false };
      case "HAS_PENDING_CART":
        return {
          ...marketingBaseWhere,
          abandonedCartJob: { is: { status: "PENDING" } },
        };
      default:
        return marketingBaseWhere;
    }
  }

  const registeredWhere: Prisma.UserWhereInput = {
    ...whereForFilter(filterStatus),
    ...searchFilter,
  };

  // Compteurs par filtre (pour les chips avec badges numérotés) — comptés
  // sur la base marketing SANS recherche pour rester stables quand la
  // cliente tape.
  const [allCount, acceptsCount, refusesCount, pendingCartCount] = await Promise.all([
    prisma.user.count({ where: marketingBaseWhere }),
    prisma.user.count({ where: whereForFilter("ACCEPTS_MAILS") }),
    prisma.user.count({ where: whereForFilter("REFUSES_MAILS") }),
    prisma.user.count({ where: whereForFilter("HAS_PENDING_CART") }),
  ]);

  const totalCount = allCount;
  const cardsTotalCount = await prisma.adminClientCard.count({});

  // Count filtré = respecte AUSSI la recherche.
  const filteredRegisteredCount = registeredSearch
    ? await prisma.user.count({ where: registeredWhere })
    : filterStatus === "ACCEPTS_MAILS"
      ? acceptsCount
      : filterStatus === "REFUSES_MAILS"
        ? refusesCount
        : filterStatus === "HAS_PENDING_CART"
          ? pendingCartCount
          : allCount;

  const counts: Record<string, number> = {
    ALL: allCount,
    ACCEPTS_MAILS: acceptsCount,
    REFUSES_MAILS: refusesCount,
    HAS_PENDING_CART: pendingCartCount,
  };
          ...
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
            <div>
              <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/70 backdrop-blur border border-border text-[11px] font-body font-bold uppercase tracking-[0.18em] text-violet-800">
                <span className="w-1.5 h-1.5 rounded-full bg-violet-500 shadow-[0_0_0_3px_rgba(139,92,246,0.14)]" />
                Marketing
              </span>
            ...
            <Link
              ...
              className="group relative inline-flex items-center gap-2.5 px-5 py-3 rounded-xl text-sm font-body font-bold bg-gradient-to-br from-violet-600 to-violet-700 text-white shadow-md hover:shadow-lg hover:opacity-95 transition-all whitespace-nowrap self-start lg:self-auto"
            >
              <span className="text-lg">📢</span>
              Gérer mes modèles de mail
              <svg className="w-4 h-4 opacity-80 group-hover:translate-x-0.5 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2">
          {FILTERS.map((filter) => {
            ...
            const isCartChip = filter.value === "HAS_PENDING_CART";
            const isRefusesChip = filter.value === "REFUSES_MAILS";
            ...
            if (isActive) {
              ...
              } else if (isRefusesChip) {
                chipClass = "bg-gradient-to-br from-slate-600 to-slate-700 border-slate-600 text-white shadow-sm";
                countClass = "bg-white/20 text-white";
              } else {
                chipClass = "bg-gradient-to-br from-violet-600 to-violet-700 border-violet-600 text-white shadow-sm";
                countClass = "bg-white/20 text-white";
              }
            } else if (isCartChip && count > 0) {
              // Panier abandonné en cours : highlight amber en veille pour attirer l'œil
      ...
      {clients.length === 0 ? (
        <div className="bg-bg-primary rounded-2xl border border-border shadow-sm py-16 px-6 text-center">
          ...
          <p className="text-sm text-text-muted max-w-md mx-auto">
            {search
              ? `Aucun résultat pour « ${search} »${filterStatus === "ALL" ? "" : ` dans le segment « ${FILTERS.find(f => f.value === filterStatus)?.label ?? ""} »`}.`
              : filterStatus === "ALL"
                ? "Aucun client approuvé n'est encore enregistré."
                : `Aucun client dans le segment « ${FILTERS.find(f => f.value === filterStatus)?.label ?? ""} » pour l'instant.`}
          </p>
          {(search || filterStatus !== "ALL") && (
            <Link href="/admin/marketing" className="btn-ghost mt-6 inline-flex">
              ← Voir tous les clients approuvés
            </Link>
          )}
        </div>
      ) : view === "mails" ? (
              ...
              <table className="w-full">
                <thead className="bg-bg-secondary">
                  <tr className="border-b border-border">
                    ...
                    <th className="px-5 py-3 text-left text-[11px] font-body font-bold text-text-muted uppercase tracking-[0.12em] whitespace-nowrap">Prochaine relance</th>
                ...
                <tbody>
                  {clients.map((c) => {
                    ...
                    const abandonedJob = abandonedJobs.get(c.id);
                    return (
                      <tr
                        key={c.id}
                        className={`border-b border-border last:border-0 transition-colors hover:bg-bg-secondary/60 ${
