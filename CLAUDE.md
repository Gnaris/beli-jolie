...
**Raccourci deploy rapide** — `scripts/deploy/deploy-fast.ps1` (PowerShell) encapsule tout le workflow (pré-flight → backup → build local → SCP `.next` → sync VPS → vérif visiteur). Le build tourne sur le PC (2-3 min) au lieu du VPS (5-10 min), on ne transfère que le `.next/` compilé. **Toujours proposer ce script à la cliente** quand elle dit « push en prod » — sauf si elle demande explicitement l'ancien workflow VPS-build. Ok grâce à `serverExternalPackages` (sharp/pdfkit/playwright/exceljs pas bundlés → chargés depuis le `node_modules` Linux du VPS au runtime, pas de souci de binaire Windows vs Linux). Rollback : le script imprime les commandes exactes à la fin.
