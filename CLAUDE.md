...
- **Kill switch marketplaces — 2 dimensions (2026-08-18)** : chaque marketplace (PFS, Ankorstore, eFashion, Faire, Microstore) a 2 toggles distincts dans `/admin/parametres` onglet Marketplaces :
  - **« Gestion Produits »** (SiteConfig `{marketplace}_products_management_enabled`, défaut ON) : quand OFF, aucun push/publish/refresh/resync/synchro/delete ne part ; badge grisé partout ; cases masquées dans les modales publish/refresh. Lecture via `getCachedXEnabled()` (sémantique = hasConfig ∧ productsManagementEnabled).
  - **« Gestion Commandes »** (SiteConfig `{marketplace}_orders_worker_enabled`, défaut ON) : quand OFF, le worker de sync commandes skip silencieusement ce tenant. Lecture via `isMarketplaceAutoSyncEnabled(tid, source)` (`lib/marketplace-auto-sync.ts`).
  - Server actions : `setMarketplaceProductsManagement(marketplace, enabled)` et `setMarketplaceAutoSyncEnabled({source, enabled})`. Anciens `togglePfsEnabled/toggleAnkorstoreEnabled/…` supprimés.
  - Migration one-shot : `npx tsx scripts/migrate-marketplace-toggles.ts --apply` (copie `X_enabled` → `X_products_management_enabled` puis supprime l'ancienne clé).
...
- **PFS fallback compo (mobile API)** : PFS expose deux APIs distinctes — `wholesaler-api.parisfashionshops.com` (utilisée par notre import/audit, OAuth) et `admin.parisfashionshops.com` (utilisée par l'appli mobile « PFS - Back Office Grossiste », Bearer via `/api/auth/seller`). Bug racine côté PFS : quand un vendeur crée un produit via l'appli mobile, la synchro wholesaler ↔ admin est incomplète sur `material_composition` → l'API wholesaler renvoie `[]` alors que la compo est bien saisie. Fallback implémenté dans `lib/pfs-admin-api.ts` : `pfsAdminFetchMaterialComposition(pfsProductId)` relit la compo côté admin quand le wholesaler renvoie vide. Déclenché dans `lib/pfs-import.ts` (approveAndImportPfsProduct), `lib/pfs-verify.ts` (verifyPfsProduct) et `lib/pfs-verify-apply.ts` (`applyPfsVerifyPullsOnly` / `applyPfsVerifyActions` — via `enrichCheckRefCompositionIfEmpty`). Pull auto composition dispo depuis 2026-08-01 : `resolvePfsCompositionsToLocal` matérialise les ProductComposition en base (auto-création via `createOrLinkMapping` si code PFS inconnu localement, dédoublonnage/merge des % si 2 codes mappent sur la même Composition). Mêmes identifiants que l'API wholesaler (`pfs_email` + `pfs_password` en SiteConfig). Constante `AemikSEAUID = "AemikWeb3_PFSBKOFFICE"` requise dans tous les POST admin. **Doc complète endpoints/format** : `docs/pfs-mobile-api.md`.
