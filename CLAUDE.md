...
- **PFS fallback compo (mobile API)** : PFS expose deux APIs distinctes — `wholesaler-api.parisfashionshops.com` (utilisée par notre import/audit, OAuth) et `admin.parisfashionshops.com` (utilisée par l'appli mobile « PFS - Back Office Grossiste », Bearer via `/api/auth/seller`). Bug racine côté PFS : quand un vendeur crée un produit via l'appli mobile, la synchro wholesaler ↔ admin est incomplète sur `material_composition` → l'API wholesaler renvoie `[]` alors que la compo est bien saisie. Fallback implémenté dans `lib/pfs-admin-api.ts` : `pfsAdminFetchMaterialComposition(pfsProductId)` relit la compo côté admin quand le wholesaler renvoie vide. Déclenché dans `lib/pfs-import.ts` (approveAndImportPfsProduct) et `lib/pfs-verify.ts` (verifyPfsProduct). Mêmes identifiants que l'API wholesaler (`pfs_email` + `pfs_password` en SiteConfig). Constante `AemikSEAUID = "AemikWeb3_PFSBKOFFICE"` requise dans tous les POST admin.
