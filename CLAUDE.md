...
**⛔ Règle absolue — audit PFS auto ≠ refresh** (incident 14/09/2026, 189 fiches eFashion + 189 Faire recréées en 24 h sur Issyma) : l'audit PFS auto (`pfs-audit-runner.ts::enqueueMarketplacePropagation`) et **toute autre automatisation** (scheduler, cron, worker, post-import) qui propage vers les marketplaces doit utiliser `mode="RESYNC"` (update in place avec `forceFullSync:true`), **JAMAIS `mode="REFRESH"`**. Sur eFashion et Faire, le refresh fait un delete + recreate qui casse URLs, favoris et stats acheteuses. Le bouton « Rafraîchir » manuel reste en `REFRESH` — seule la cliente décide quand un produit remonte en nouveauté. Test anti-régression : `__tests__/lib/pfs-audit-runner-propagation-mode.test.ts`.

**Refs temporaires — filet de sécurité (2026-09-14)** : les refs temporaires posées pendant un refresh incluent la vraie référence BJ du produit pour que toute fiche orpheline soit identifiable visuellement. Format : `DEL-{ref_bj}-{random4}-{couleur}` sur eFashion (`lib/efashion-refresh.ts` + `-batch.ts`), `TMP-{ref_bj}-{random6}` sur PFS (`lib/pfs-refresh.ts`), `{nom} [REFRESH_{ref_bj}_{ts}]` sur Faire (`lib/faire-refresh.ts`). Ne pas revenir aux UUID nus.
...
### Arrondi commande / facture — règle Sage 50 (2026-09-01)
Depuis le 2026-09-01, tout l'arrondi monétaire (checkout, snapshot commande, recalcul post-rupture, PDF facture/avoir, emails) passe par `roundCent` (`lib/money.ts`) — arrondi **au centime le plus proche** (round half up) + correction `Number.EPSILON` pour absorber les artefacts IEEE-754. Avant on utilisait `Math.floor` (vers le bas) censé matcher le logiciel externe : en réalité Sage 50 arrondit au plus proche, ce qui créait 1 centime d'écart chronique par facture. Vérifié sur facture Sage FA10005485. **Règle** : Remise = `roundCent(HT × taux)`, Net HT = HT − Remise, TVA = `roundCent(NetHT × tvaRate)`, TTC = NetHT + TVA (**pas de re-arrondi**). Callers migrés : `lib/order-pricing.ts` (checkout + Stripe), `lib/order-totals.ts` (recalcul rupture), `lib/notifications.ts` (emails), 2 routes PDF (`app/api/{admin,client}/commandes/[id]/pdf/route.ts`), 2 pages fiche commande, `components/admin/orders/OrderContent.tsx`. Marketplaces non touchées (leur `Math.ceil` au dixième reste pertinent). Commandes historiques figées en BDD non modifiées (subtotalHT/tvaAmount/totalTTC restent tels quels, seul le recalcul post-modif re-persiste avec le nouvel arrondi). **Résumé admin** affiche ancien montant barré → nouveau sur les 5 lignes (Sous-total HT, Remise, Total après remise, TVA, TTC) quand rupture ou compensation détectée.
