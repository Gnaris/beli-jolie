 ...
 *
 * Le rattrapage historique boucle sur toutes les pages Ankorstore par curseur.
 * L'état est stocké en SiteConfig pour survivre à un redémarrage PM2 et être
...
export interface AnkorstoreImportState {
  ...
  /** Ankorstore ne fournit pas de total → null tant qu'on n'a pas fini. */
...
async function setState(tenantId: string, state: AnkorstoreImportState): Promise<void> {
...
async function patchState(
  ...
  await setState(tenantId, { ...current, ...patch });
}
...
async function pushEvent(
  ...
  await setState(tenantId, { ...current, recentEvents: next });
}
...
export async function startAnkorstoreHistoricalImportInBackground(
  ...
  await setState(tenantId, initial);
  await setStopSignal(tenantId, false);

  void tenantALS.run(tenantId, async () => {
    try {
      const { importAllAnkorstoreOrdersFor } = await import("@/lib/ankorstore-orders-sync");
      const result = await importAllAnkorstoreOrdersFor(
        ...
        },
        {
          ...
          onEvent: (event) => pushEvent(tenantId, event),
        },
      );
      ...
      await setState(tenantId, finalState);
      await setStopSignal(tenantId, false);
    } catch (err) {
      ...
      await setState(tenantId, errorState);
    }
  });
...
export async function requestStopAnkorstoreHistoricalImport(tenantId: string): Promise<void> {
  await setStopSignal(tenantId, true);
}

export async function resetAnkorstoreImportState(tenantId: string): Promise<void> {
  await setState(tenantId, { ...EMPTY_STATE });
  await setStopSignal(tenantId, false);
}
