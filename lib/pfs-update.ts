...
function buildVariantCreateData(
  ...
  const isActive = pfsComputeVariantIsActive(variant);
...
function buildProductFieldsSnapshot(
  ...
  return {
    ...
    composition,
    country: product.countryIsoCode ?? "CN",
...
function buildVariantSnapshot(
  ...
  return {
    ...
    stock,
    weight: variant.weight,
    isActive: pfsComputeVariantIsActive(variant),
    colorRef: effective,
  ...
  try {
    ...
    for (const variant of product.colors) {
      if (variant.pfsVariantId) {
        nextVariantsSnap[variant.pfsVariantId] = buildVariantSnapshot(
          variant,
          pfsMarkup,
          colorRefMap,
        );
    ...
    for (const item of variantsToUpdate) {
      ...
      if (colorChanged || saleTypeChanged || packSignatureChanged) {
        const pfsData = buildVariantCreateData(
          item.bjVariant,
          colorRefMap,
          pfsMarkup,
        );
    ...
    if (variantsToPatch.length > 0) {
      const patches: PfsVariantUpdateData[] = variantsToPatch.map(({ bjVariant, pfsVariantId }) => ({
        ...
        is_active: pfsComputeVariantIsActive(bjVariant),
      }));
    ...
    if (variantsToRecreate.length > 0) {
      ...
      for (const item of variantsToRecreate) {
        ...
        try {
          ...
          // Si stock = 0 sur la nouvelle variante, repatche stock=0 (PFS peut
          // réécrire à 300). `is_active` reste tel quel côté PFS.
          if (item.pfsData.stock_qty === 0) {
            try {
              await pfsPatchVariants([{ variant_id: newId, stock_qty: 0 }]);
            } catch { /* ignore */ }
          }
    ...
    if (variantsToCreate.length > 0) {
      ...
      try {
        ...
        // PFS peut réécrire stock=300 lors du create quand stock_qty=0 →
        // repatche systématique. `is_active` reste tel quel côté PFS.
