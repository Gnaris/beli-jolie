 ...
 *
 * Champs : Prenom, Nom, Societe, Email, Telephone, SIRET, Mot de passe,
...
export default function RegisterForm() {
  ...
  /** Met a jour un champ et efface son erreur */
  ...
  function handleKbisChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!allowedTypes.includes(file.type)) {
      setKbisError("Format invalide. Accepte : PDF, JPG, PNG (max 5 Mo).");
      setKbisFile(null);
      return;
    }

    if (file.size > 5 * 1024 * 1024) {
      setKbisError("Le fichier depasse la limite de 5 Mo.");
      setKbisFile(null);
      return;
    }
  ...
  async function handleSubmit(e: React.SyntheticEvent) {
    ...
    try {
      ...
      setGlobalError("Une erreur reseau est survenue. Veuillez reessayer.");
    } finally {
      setLoading(false);
    }
  }

  /** Rendu du message de succes apres inscription */
  if (successMessage) {
    return (
      <div className="w-full max-w-lg text-center">
        <div className="bg-bg-primary rounded-xl border border-border p-10 shadow-card">
          <div className="w-14 h-14 bg-bg-secondary rounded-full flex items-center justify-center mx-auto mb-5">
            <svg className="w-7 h-7 text-success" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
    ...
    <div className="w-full max-w-2xl">
      ...
      <div className="mb-8">
        ...
        <p className="mt-1.5 font-[family-name:var(--font-roboto)] text-sm text-text-muted">
          Remplissez le formulaire — notre equipe examinera votre dossier sous 48h.
        </p>
      </div>
      ...
      <div className="bg-bg-primary rounded-xl border border-border p-6 md:p-8 shadow-card">
        <form onSubmit={handleSubmit} noValidate encType="multipart/form-data">
          ...
          {globalError && (
            <div
              role="alert"
              className="bg-red-50 border border-red-200 text-error px-4 py-3 text-sm font-[family-name:var(--font-roboto)] flex items-start gap-2.5 mb-6 rounded-lg"
            >
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="space-y-4">
                ...
                {/* Societe */}
                <FormField
                  id="company" label="Societe" type="text"
            ...
            <div className="border-t border-border pt-6">
              ...
              <div className="space-y-4">
                {/* SIRET */}
                <FormField
                  id="siret" label="Numero SIRET" type="text"
                ...
                <div>
                  ...
                  <p className="text-xs text-text-muted mt-1 font-[family-name:var(--font-roboto)]">
                    Requis pour les entreprises UE hors France (exoneration TVA par autoliquidation)
                  </p>
                  {fieldErrors.vatNumber && (
                    <p className="text-xs text-error mt-1 font-[family-name:var(--font-roboto)]">{fieldErrors.vatNumber}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <div
                    ...
                    role="button"
                    tabIndex={0}
                    aria-label="Selectionner le fichier Kbis"
                  >
                    <input
                      ref={fileInputRef}
                      id="kbis"
                    ...
                    {kbisFile ? (
                      ...
                      <>
                        <svg className="w-8 h-8 text-text-muted mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" />
                        </svg>
                        <p className="text-sm font-[family-name:var(--font-roboto)] text-text-secondary">
                          Cliquez pour selectionner votre Kbis
                        </p>
                        <p className="text-xs text-text-muted mt-1">PDF, JPG, PNG — max 5 Mo</p>
                      </>
                    )}
                  </div>
                  {kbisError && (
                    <p className="text-xs text-error mt-1 font-[family-name:var(--font-roboto)]" role="alert">
                      {kbisError}
                    </p>
                  )}
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label
                    ...
                    className="block text-[13px] font-[family-name:var(--font-roboto)] font-medium text-text-secondary mb-1.5"
                  >
                    Mot de passe <span className="text-text-muted">*</span>
                  </label>
                  ...
                  {fieldErrors.password && <p className="text-xs text-error mt-1 font-[family-name:var(--font-roboto)]">{fieldErrors.password}</p>}
                  <p className="text-xs text-text-muted mt-1 font-[family-name:var(--font-roboto)]">8 caracteres min, 1 majuscule, 1 chiffre</p>
                </div>
...
// ─────────────────────────────────────────────
// Composant champ de formulaire reutilisable
// ─────────────────────────────────────────────
function FormField({
  ...
  return (
    <div>
      ...
      />
      {error && (
        <p id={`${id}-error`} role="alert" className="text-xs text-error mt-1 font-[family-name:var(--font-roboto)]">
          {error}
        </p>
      )}
    </div>
