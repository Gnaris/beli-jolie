...
export default function ColorMarketplaceMappingCards({
  pfsLabel,
  pfsSharedCount,
  efashionLabel,
  microstoreLabel,
        ...
        return (
          ...
          >
            ...
            <div className={`text-[11.5px] md:text-[12.5px] leading-snug min-h-[26px] md:min-h-[18px] flex items-center gap-1.5 flex-wrap ${mapped ? "text-text-primary font-medium" : "text-text-muted italic"}`}>
              ...
              {m.key === "pfs" && mapped && pfsSharedCount > 0 && (
                <span
                  className="inline-flex items-center gap-1 text-[10px] font-normal not-italic text-text-muted bg-bg-secondary border border-border rounded-full px-2 py-0.5"
                  title={`Ce mapping PFS est partagé avec ${pfsSharedCount} autre${pfsSharedCount > 1 ? "s" : ""} couleur${pfsSharedCount > 1 ? "s" : ""}. Pas bloquant.`}
                >
                  ⓘ partagé (×{pfsSharedCount + 1})
                </span>
              )}
            </div>
            <button
