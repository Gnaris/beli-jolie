...
export default function ColorsManager({ initialColors }: { initialColors: ColorItem[] }) {
  ...
  function handleDelete(color: ColorItem) {
  ...
  async function handleSave(name: string, translations: Record<string, string>, hex?: string) {
    if (!editTarget) return;
    await updateColorDirect(editTarget.id, name, hex ?? null, translations);
    router.refresh();
  }
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-text-muted">
                    ...
                    {color.productCount} produit{color.productCount > 1 ? "s" : ""}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-1">
                ...
                <button
                  ...
                  className="p-1.5 text-text-muted hover:text-[#EF4444] transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
