...
type Marketplace = "pfs" | "efashion" | "faire" | "orderchamp" | "microstore";
...
export default function MarketplaceMappingCards({
  pfsLabel,
  efashionLabel,
  faireLabel,
  orderchampLabel,
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-5 gap-2.5 md:gap-3 [&>:last-child]:col-span-2 lg:[&>:last-child]:col-span-1">
      {ITEMS.map((m) => {
        ...
        return (
          ...
          >
            ...
            <div
              className={`text-[11.5px] md:text-[12.5px] leading-snug min-h-[26px] md:min-h-[18px] line-clamp-2 md:line-clamp-none ${
                mapped ? "text-text-primary font-medium" : "text-text-muted italic"
              }`}
            >
              {value ?? "Non mappé"}
            </div>
