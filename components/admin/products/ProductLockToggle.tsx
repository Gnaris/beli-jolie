...
export function ProductLockToggle({ productId, initialLocked, variant = "button" }: Props) {
  ...
  return (
    <button
      ...
      title={title}
      aria-pressed={locked}
      className={`inline-flex items-center gap-2 px-3.5 py-2 text-[13px] font-medium rounded-xl border transition-all font-body shadow-sm disabled:opacity-50 disabled:cursor-wait ${
        locked
          ? "text-amber-700 bg-amber-50 border-amber-200 hover:border-amber-300"
          : "text-text-secondary bg-bg-primary border-border hover:border-border-dark hover:text-text-primary"
      }`}
    >
      <LockIcon locked={locked} className="w-4 h-4" />
