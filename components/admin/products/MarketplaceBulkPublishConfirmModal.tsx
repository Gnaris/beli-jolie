...
export default function MarketplaceBulkPublishConfirmModal({
  open,
  marketplaceName,
  marketplaceCode,
  products,
  ...
  const defaultSubtitle = `${count} produit${plural} seront publiés avec les infos, photos, prix et stock actuels.`;
  const defaultInfo = (
    <>
    ...
    >
      ...
      >
        ...
        <div className={`relative overflow-hidden bg-gradient-to-br ${palette.aurora} px-6 pt-6 pb-5`}>
          ...
          <div className="relative flex items-start gap-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <h3 className="font-heading text-xl font-bold text-text-primary leading-tight">
                Publier {count} produit{plural} sur {marketplaceName}&nbsp;?
              </h3>
          ...
          {/* Carte produits — pile de miniatures + compteur */}
          <div className="relative mt-5 flex items-center gap-3 rounded-2xl bg-bg-primary/70 backdrop-blur border border-white ring-1 ring-border px-3 py-2.5">
            <div className="flex items-center shrink-0">
              {preview.map((p, idx) => (
                <div
                  key={p.id}
                  className="w-11 h-11 rounded-lg bg-bg-secondary border-2 border-white ring-1 ring-border flex items-center justify-center text-text-muted overflow-hidden"
                  style={{ marginLeft: idx === 0 ? 0 : -14, zIndex: preview.length - idx }}
                  title={p.name}
                >
                  {p.firstImage ? (
              ...
              {rest > 0 && (
                <div
                  className="w-11 h-11 rounded-lg bg-bg-secondary border-2 border-white ring-1 ring-border flex items-center justify-center text-[11px] font-bold text-text-secondary"
