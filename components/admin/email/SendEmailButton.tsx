...
export default function SendEmailButton({ recipient, variant = "button", className = "" }: SendEmailButtonProps) {
  ...
  return (
    <button
      type="button"
      onClick={() => openCompose(recipient)}
      className={`inline-flex items-center gap-2 bg-bg-dark text-text-inverse text-sm font-medium font-[family-name:var(--font-roboto)] px-4 py-2 rounded-lg hover:opacity-90 transition-opacity ${className}`}
    >
