...
export default function HsCodeDetail({
  ...
  const statusLabel = isUnused
    ? "Inutilisé"
    : item.productCount >= 100
      ? "Fortement utilisé"
      : "En circulation";
  const statusDotClass = isUnused
    ? "bg-rose-400"
    : item.productCount >= 100
      ? "bg-emerald-500"
      : "bg-sky-500";
    ...
    <div className="flex flex-col gap-4 p-5 md:p-7 bg-bg-primary overflow-y-auto md:h-full">
      ...
      <div className="flex items-start justify-between gap-3 pb-5 border-b border-border">
        <div className="flex items-start gap-4 min-w-0">
          ...
          <div
            aria-hidden
            className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#27272A] to-[#18181B] text-white flex items-center justify-center font-mono text-[13px] font-bold flex-shrink-0 tracking-tight"
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <span className="font-mono text-[10.5px] font-semibold tracking-wider text-text-muted uppercase">Numéro</span>
              <span className="font-mono text-[15px] font-bold text-text-primary tracking-tight">
            ...
            <div className="flex items-center gap-2.5 text-[12px] text-text-secondary mt-2 flex-wrap">
              ...
              <span className="inline-flex items-center gap-1">
                <span aria-hidden className={`w-1.5 h-1.5 rounded-full ${statusDotClass}`} />
                {statusLabel}
              </span>
              <span aria-hidden className="w-[3px] h-[3px] rounded-full bg-text-muted" />
        ...
        <div className="flex gap-2 flex-shrink-0">
          ...
          <button
            ...
            title={deleteBlocked ? "Impossible : ce code est attribué à des produits." : undefined}
        ...
        <section className="relative overflow-hidden rounded-2xl border border-border p-5 bg-bg-primary" style={{ boxShadow: "var(--shadow-sm)" }}>
          ...
          <div className="relative">
            ...
            <p className="text-[12.5px] text-text-secondary mb-4 font-body">
              Ces informations apparaissent dans les documents douaniers et les exports marketplaces.
            </p>
            <div className="space-y-3">
              <div>
                <div className="text-[10px] font-semibold uppercase tracking-wider text-text-muted mb-1">Numéro</div>
                <div className="font-mono font-bold text-text-primary text-[15px] tracking-tight">
                  {formatCode(item.code)}
                </div>
              </div>
              <div>
                <div className="text-[10px] font-semibold uppercase tracking-wider text-text-muted mb-1">Libellé</div>
                <div className="text-[13.5px] text-text-primary">{item.label}</div>
              </div>
            </div>
          </div>
        </section>
          ...
          <div className="relative">
            ...
            {deleteBlocked && (
              <div className="mt-4 flex items-center gap-2 text-[12px] text-text-secondary p-3 rounded-xl bg-bg-secondary border border-border">
                ...
                <span>La suppression restera bloquée tant qu&apos;au moins un produit utilise ce code.</span>
              </div>
            )}
          </div>
        </section>
