...
interface SeasonItem {
  ...
  efashionCollectionLabel?: string | null;
  productCount: number;
                ...
                {filtered.map((item) => (
                  <tr key={item.id} className="hover:bg-bg-secondary/50 transition-colors">
                    ...
                    <td className="px-4 py-3 hidden md:table-cell">
                      <MarketplaceMappingBadge value={item.pfsRef} />
                    </td>
                    {/* eFashion */}
                    <td className="px-4 py-3 hidden md:table-cell">
                      <MarketplaceMappingBadge
                        value={item.efashionCollectionLabel ?? (item.efashionCollectionId != null ? `id ${item.efashionCollectionId}` : null)}
                        title={item.efashionCollectionId != null ? `id ${item.efashionCollectionId}` : undefined}
                      />
                    </td>
