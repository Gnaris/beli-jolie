...
export default function SeasonEditorModal({
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const result = await createSeasonQuick(names);
      onCreated?.(result);
      onClose();
    } catch (e) {
  ...
  return createPortal(
    ...
    >
      ...
      >
        ...
        {/* Header aurora — bleu ciel (thème saison) */}
        <div className="relative overflow-hidden border-b border-border shrink-0">
          <div
            ...
            style={{
              background:
                "radial-gradient(60% 80% at 10% 0%, rgba(56,189,248,0.12), transparent 60%)," +
          ...
          <div className="relative px-5 sm:px-7 pt-4 sm:pt-6 pb-4 sm:pb-5">
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-start gap-3 min-w-0">
                <span className="inline-flex items-center justify-center w-11 h-11 rounded-2xl bg-bg-primary border border-border shadow-sm text-sky-600">
                ...
                <div className="min-w-0">
                  <span className="inline-flex items-center gap-2 px-2.5 py-1 rounded-full bg-sky-50 border border-sky-100 text-sky-700 text-[10.5px] font-bold uppercase tracking-[0.14em]">
                    <span className="w-1.5 h-1.5 rounded-full bg-sky-500" />
                    Catalogue · Saison
                  </span>
                  <h3 className="font-heading text-[20px] font-bold text-text-primary leading-tight mt-2 truncate">
                    {isEdit ? `Renommer « ${editMode.name} »` : "Créer une saison"}
                  </h3>
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-7 space-y-4 sm:space-y-5">
          ...
          <div>
            ...
            <input
              ...
              onBlur={handleFrBlur}
              autoFocus
              placeholder="Ex : Printemps/Été 2026, Intemporel…"
          ...
          <div className="rounded-2xl bg-bg-secondary border border-border p-4 flex gap-3">
            ...
            <p className="text-[12px] text-text-secondary leading-relaxed">
              <strong className="text-text-primary">Prochaine étape</strong> — {isEdit
                ...
                : "après création, clique sur la nouvelle saison puis sur chaque carte marketplace pour la relier."}
            </p>
          </div>
        </div>
            ...
            >
              {loading ? (
                ...
                <>
                  ...
                  {isEdit ? "Enregistrer" : "Créer la saison"}
                </>
              )}
            </button>
          </div>
