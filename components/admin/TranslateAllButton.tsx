...
export default function TranslateAllButton({
  ...
  async function handleClick() {
    ...
    if (remaining < totalChars) {
      ...
      setError(
        `Quota insuffisant (${remaining.toLocaleString("fr-FR")} restants, ${totalChars.toLocaleString("fr-FR")} nécessaires). Réinitialisation le ${formatted}.`
      );
      return;
    }
    ...
    setLoading(true);
    setProgress({ current: 0, total: toTranslate.length });
    ...
    try {
      ...
      for (let i = 0; i < toTranslate.length; i += BATCH_SIZE) {
        ...
        setProgress({ current: completed, total: toTranslate.length });
      }
