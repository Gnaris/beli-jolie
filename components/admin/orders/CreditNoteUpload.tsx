...
export default function CreditNoteUpload({ orderId, hasCreditNote }: Props) {
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setUploading(async () => {
      try {
        const res = await fetch(`/api/admin/commandes/${orderId}/credit-note`, {
          method: "POST",
          body: formData,
        });
        if (!res.ok) {
  ...
  async function handleDelete() {
    ...
    setDeleting(async () => {
      try {
        const res = await fetch(`/api/admin/commandes/${orderId}/credit-note`, { method: "DELETE" });
        if (!res.ok) {
          const data = await res.json().catch(() => ({}));
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          ...
          {uploading ? "Upload..." : hasCreditNote ? "Remplacer l'avoir" : "Uploader un avoir"}
          <input
            ref={inputRef}
            type="file"
        ...
        {hasCreditNote && (
          <>
            <a
              href={`/api/admin/commandes/${orderId}/credit-note`}
              target="_blank"
      ...
      {hasCreditNote && !error && (
        <p className="text-xs text-emerald-600 font-body flex items-center gap-1.5">
          ...
          Avoir disponible — le client peut le telecharger
        </p>
      )}
      {!hasCreditNote && !error && (
        <p className="text-xs text-text-muted font-body">
          Aucun avoir uploade pour cette commande
        </p>
      )}
      {error && (
