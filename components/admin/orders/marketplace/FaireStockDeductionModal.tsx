...
import {
  previewFaireOrderStockDeduction,
  runFaireOrderStockDeductionOne,
  type FaireSingleOrderDeductionPreview,
...
const SKIP_REASON_LABELS: Record<string, string> = {
  ...
  VARIANT_NOT_LINKED: "Couleur non rattachée à votre boutique",
  NO_SIZE_MATCH:
    "Plusieurs tailles côté boutique — impossible de choisir (Faire ne renvoie pas la taille séparément)",
  QTY_ZERO: "Quantité à zéro",
};

type Preview = NonNullable<FaireSingleOrderDeductionPreview>;

export default function FaireStockDeductionModal({ orderId, onClose, onDeducted }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      setError(null);
      const res = await previewFaireOrderStockDeduction(orderId);
      if (cancelled) return;
      if (!res.success) {
        setError(res.error);
        setLoading(false);
  ...
  const handleRun = async () => {
    setRunning(true);
    const res = await runFaireOrderStockDeductionOne(orderId);
    setRunning(false);
    if (!res.success) {
    ...
    if (res.processedCount === 0) {
      warning(
        "Rien n'a été déduit",
        "Aucune ligne n'a pu être traitée (couleurs non rattachées ou plusieurs tailles).",
      );
    } else {
      success(
        "Stock déduit",
        ...
        <div className="px-6 py-5 border-b border-border flex items-center justify-between gap-4">
          <div className="min-w-0">
            ...
            <h2 className="font-heading text-xl font-bold text-text-primary mt-0.5 truncate">
              Commande Faire <span className="font-mono">{preview?.orderNumber ?? "…"}</span>
            </h2>
          </div>
          <button
            type="button"
                ...
                <div className="mt-5 rounded-xl bg-bg-secondary border border-border p-4">
                  ...
                  <ul className="space-y-1.5 text-[12.5px]">
                    {skippedLines.map((line) => (
                      <li key={line.faireOrderItemId} className="flex items-baseline gap-2">
