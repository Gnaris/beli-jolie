...
import {
  previewEfashionOrderStockDeduction,
  runEfashionOrderStockDeductionOne,
  type EfashionSingleOrderDeductionPreview,
...
const SKIP_REASON_LABELS: Record<string, string> = {
  ...
  NO_SIZE_MATCH: "Aucune taille ne correspond au libellé eFashion",
  QTY_ZERO: "Quantité à zéro",
};

export default function EfashionStockDeductionModal({ orderId, onClose, onDeducted }: Props) {
  const [preview, setPreview] = useState<EfashionSingleOrderDeductionPreview | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      setError(null);
      const res = await previewEfashionOrderStockDeduction(orderId);
      if (cancelled) return;
      if (!res.success) {
        setError(res.error);
        setLoading(false);
  ...
  const handleRun = async () => {
    setRunning(true);
    const res = await runEfashionOrderStockDeductionOne(orderId);
    setRunning(false);
    if (!res.success) {
        ...
        <div className="px-6 py-5 border-b border-border flex items-center justify-between gap-4">
          <div className="min-w-0">
            ...
            <h2 className="font-heading text-xl font-bold text-text-primary mt-0.5 truncate">
              Commande eFashion <span className="font-mono">{preview?.orderNumber ?? "…"}</span>
            </h2>
          </div>
          <button
            type="button"
                      ...
                      <div className="flex items-start justify-between gap-3 flex-wrap">
                        <div className="min-w-0">
                          ...
                          <div className="text-[11.5px] text-text-muted mt-0.5">
                            ...
                            {" · "}
                            {line.qtyTotal} unité{line.qtyTotal > 1 ? "s" : ""}
                          </div>
                        </div>
                      </div>
                ...
                <div className="mt-5 rounded-xl bg-bg-secondary border border-border p-4">
                  ...
                  <ul className="space-y-1.5 text-[12.5px]">
                    {skippedLines.map((line) => (
                      <li key={line.efashionOrderItemId} className="flex items-baseline gap-2">
                        <span className="font-mono text-text-muted shrink-0">{line.reference}</span>
