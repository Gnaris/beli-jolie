...
import {
  previewAnkorstoreOrderStockDeduction,
  runAnkorstoreOrderStockDeductionOne,
  type AnkorstoreSingleOrderDeductionPreview,
...
const SKIP_REASON_LABELS: Record<string, string> = {
  ...
  NO_SIZE_MATCH:
    "Plusieurs tailles côté boutique — impossible de choisir (Ankor ne renvoie pas la taille)",
  QTY_ZERO: "Quantité à zéro",
};

type Preview = NonNullable<AnkorstoreSingleOrderDeductionPreview>;

export default function AnkorstoreStockDeductionModal({ orderId, onClose, onDeducted }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      setError(null);
      const res = await previewAnkorstoreOrderStockDeduction(orderId);
      if (cancelled) return;
      if (!res.success) {
        setError(res.error);
        setLoading(false);
  ...
  const handleRun = async () => {
    setRunning(true);
    const res = await runAnkorstoreOrderStockDeductionOne(orderId);
    setRunning(false);
    if (!res.success) {
        ...
        <div className="px-6 py-5 border-b border-border flex items-center justify-between gap-4">
          <div className="min-w-0">
            ...
            <h2 className="font-heading text-xl font-bold text-text-primary mt-0.5 truncate">
              Commande Ankorstore <span className="font-mono">#{preview?.orderNumber ?? "…"}</span>
            </h2>
          </div>
          <button
            type="button"
              ...
              {eligibleLines.length === 0 ? (
                ...
                <ul className="space-y-3">
                  {eligibleLines.map((line) => (
                    <li
                      key={line.ankorstoreOrderItemId}
