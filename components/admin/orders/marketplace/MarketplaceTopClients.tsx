...
export default function MarketplaceTopClients({ stats }: Props) {
  ...
  const allRows = useMemo(() => {
    if (!stats) return [];
    // Filtre marketplace (intersection : le client doit être présent sur toutes
    // les marketplaces cochées). Aucune case cochée = pas de filtre.
    const filtered = sourceFilter.length === 0
      ? stats.topClients
      : stats.topClients.filter((c) =>
          sourceFilter.every((src) => c.sources.includes(src)),
        );
    ...
    <div className="relative rounded-2xl bg-bg-primary border border-border shadow-sm p-5 flex flex-col h-full">
        ...
        <p className="text-sm text-text-muted py-6 text-center">
          {sourceFilter.length > 0
            ? sourceFilter.length === 1
              ? `Aucun client sur cette marketplace pour la période.`
              : `Aucun client actif sur les ${sourceFilter.length} marketplaces cochées pour la période.`
      ...
      <ul className="divide-y divide-border flex-1 flex flex-col">
        {rows.map((c, i) => {
            ...
            <>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-1.5 text-[11.5px] text-text-muted truncate">
                  ...
                  Dernière : {formatLastOrder(c.lastOrderAt, nowMs)}
                </div>
              </div>
          ...
          return (
            <li key={c.key} className="flex-1 flex">
              {c.adminClientCardId ? (
                <Link
