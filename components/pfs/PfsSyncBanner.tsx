...
export default function PfsSyncBanner({
  ...
  const [status, setStatus] = useState<BannerStatus>(() => {
    ...
    return pfsProductId ? "checking" : "not_on_pfs";
  });
  ...
  }, [pfsProductId, runCheck]);

  // Auto-check on mount if product is on PFS
  useEffect(() => {
    if (!pfsProductId || (mappingIssues && mappingIssues.length > 0)) return;
    if (pfsSyncStatus === "failed") return; // Show error state from DB
        ...
        <div className="flex items-center gap-2">
          ...
          {pfsProductId && (
            <button
              type="button"
              onClick={runCheck}
              className="inline-flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium text-blue-700 hover:text-blue-900 border border-blue-300 bg-blue-100 hover:bg-blue-200 rounded-lg transition-colors"
            >
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182" />
              </svg>
      ...
      <div className="px-4 py-3 bg-red-50 border border-red-200 rounded-xl text-sm font-body space-y-2">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={pfsProductId ? runCheck : handleCreateOnPfs}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-white bg-[#1A1A1A] hover:bg-black rounded-lg transition-colors"
            >
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182" />
