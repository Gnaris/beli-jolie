...
export default function PfsMappingClient({ colors: initialColors, categories: initialCategories, compositions: initialCompositions, countries: initialCountries, seasons: initialSeasons, sizes: initialSizes, multiColorVariants: initialMultiColorVariants }: Props) {
  ...
  const handleSaveSeason = useCallback(async (seasonId: string, pfsRef: string | null) => {
    setSaving(seasonId);
    try {
      await updateSeasonPfsRef(seasonId, pfsRef);
      setSeasons((prev) => prev.map((s) => (s.id === seasonId ? { ...s, pfsRef } : s)));
    } catch (err) {
      alert(err instanceof Error ? err.message : "Erreur");
    }
    setSaving(null);
  ...
  const tabs: { key: Tab; label: string; count: number; mapped: number }[] = [
    ...
    { key: "seasons", label: "Saisons", count: seasons.length, mapped: seasons.filter((s) => s.pfsRef).length },
  ...
  const usedSeasonRefs = new Set(seasons.map((s) => s.pfsRef).filter(Boolean));
  ...
  return (
    <div className="space-y-4">
                    ...
                    <CustomSelect
                      ...
                      options={[
                        ...
                        ...pfsCollections.map((pc) => {
                          const taken = usedSeasonRefs.has(pc.reference) && season.pfsRef !== pc.reference;
                          return {
                            value: pc.reference,
                            label: `${pc.labels?.fr ?? pc.reference} (${pc.reference})${taken ? " ✗ déjà liée" : ""}`,
                            disabled: taken,
      ...
      <div className="flex flex-wrap gap-4 text-sm text-text-secondary">
        ...
        <span>Saisons liées : {seasons.filter((s) => s.pfsRef).length}/{seasons.length}</span>
